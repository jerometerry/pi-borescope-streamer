...
static bool up_check_ghost_header(const u8 *buffer, size_t len, size_t index,
				  size_t *hdr_off)
{
	size_t limit = (len - index - 3 < 160) ? len - index - 3 : 160;
	for (size_t o = UP_PKT_HDR_SIZE; o <= limit; o++) {
		if (up_is_valid_pkt_header(
			    (const struct up_pkt_hdr *)(buffer + index + o))) {
			*hdr_off = o;
			return true;
		}
	}
...
size_t up_parser_feed(struct up_parser *parser, const u8 *buffer, size_t len)
{
	...
	while (index + TOTAL_USB_HEADER_SIZE <= len) {
		const struct up_pkt_hdr *pkt_hdr =
			(const struct up_pkt_hdr *)(buffer + index);
		size_t pkt_len = UP_LE16_TO_CPU(pkt_hdr->le_length);
		size_t total_size = UP_PKT_HDR_SIZE + pkt_len;
		size_t ghost_off = 0;
		...
		if (pkt_len >= UP_PL_HDR_SIZE) {
			...
			if (up_valid_mjpeg_payload(pl_hdr)) {
				...
				if (parser->cb.on_video_payload) {
					const u8 *pl_src =
						buffer + index +
						TOTAL_USB_HEADER_SIZE;
					size_t pl_size = total_size -
							 TOTAL_USB_HEADER_SIZE;
					parser->cb.on_video_payload(
						parser->ctx, pl_src, pl_size);
				}
			}
		}
		index += total_size;
