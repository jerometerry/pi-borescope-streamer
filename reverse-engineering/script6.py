...
def parse_jpeg_dimensions(data_bytes):
    """Slices custom 12-byte headers and scans for the JPEG SOF0 resolution marker."""
    b = list(data_bytes)

    # Define your exact C struct lengths
    USB_FRM_HDR_LEN = 5
    FRAG_HDR_LEN = 7
    TOTAL_HEADER_LEN = USB_FRM_HDR_LEN + FRAG_HDR_LEN # 12 bytes

    # Strip the leading 12 bytes to get to the true start of the frame data
    if len(b) > TOTAL_HEADER_LEN:
        payload_data = b[TOTAL_HEADER_LEN:]
    else:
        return None

    # Scan the stripped data for the JPEG Start of Frame 0 marker (0xFF, 0xC0)
    for i in range(len(payload_data) - 8):
        ...
        # Optional fallback: If the packet is fragmented mid-stream, search the whole block
        if payload_data[i] == 0xFF and payload_data[i+1] == 0xD8:
            print("--> Found JPEG SOI (Start of Image) marker in payload data block.")
...
# 2. Control Negotiation Pipeline Setup
payload = [0] * 26
payload[0] = 0x00
payload[1] = 0x00
