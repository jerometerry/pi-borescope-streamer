...
/**
 * @brief MjpegFrameQueue is a thread safe data structure that stores the latest MJPEG frame from
 * the USB camera feed, overwriting the previous frame.
 *
 * @details MjpegStream fires new frames in the onFrameReady callback, held in a BufferPtr (aka
 * IntrusivePtr<Buffer>). When running the mjpeg_server application, onFrameReady is wired up to
 * send the latest frame into MjpegFrameQueue via push(). MjpegServer is wired up with FrameSource
 * that reads the latest frame from MjpegFrameQueue via pop().
 *
 * <h3>Example:</h3>
 * <pre><code>
 * int main() {
 ...
 * { if (status == UsbTransferStatus::Completed) { if (!payload.empty()) { stream.send(payload);
 *             }
 *             return true;
 *         }
...
class MjpegFrameQueue : public std::enable_shared_from_this<MjpegFrameQueue> {
   public:
     ...
     * transferred via std::move, and the queue's internal state becomes empty until the next
     * push().
     *
     * @param[out] outFrameId Populates with the internal sequential ID of the queue's current
     * state, even if the queue is currently empty.
