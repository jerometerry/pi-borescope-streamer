#include "buffered_mjpeg_server.hpp"
...
BufferedMjpegServer::~BufferedMjpegServer() {
...
void BufferedMjpegServer::onTimer(us_timer_t* t) {
    auto* server = *static_cast<BufferedMjpegServer**>(us_timer_ext(t));
    ...
    if (currentFrame && !currentFrame->empty() &&
        currentFrameId != server->lastBroadcastedFrameId_) {
            ...
            if (viewer.lastSentFrameId < currentFrameId) {
                ...
                if (backpressure == 0) {
                    if (viewer.isLagging) {
                        ...
                        std::cerr
                            << "[Network Telemetry] Viewer recovered. TCP pipe cleared. "
                            << droppedFrames
                            << " frames were deliberately dropped to maintain real-time latency.\n";
                        viewer.isLagging = false;
                    }

                    CorkState state{
                        res, BufferedZeroAllocationResponseBuilder::build(currentFrame.get()),
                        false};
                    ...
                    if (!state.ok) {
                        std::cerr << "[Network Telemetry] ALERT: Kernel buffer rejected data! "
                                  << "uWebSockets just executed a user-space malloc to queue this "
                                     "frame.\n";
                    }
                    ...
                    if (postWriteBackpressure > 0) {
                        std::cerr
                            << "[Network Telemetry] HEAP ALLOCATION DETECTED! res->write() caused "
                            << postWriteBackpressure
                            << " bytes to be queued on the heap for viewer frame " << currentFrameId
                            << "\n";
                    }
                } else {
                    if (!viewer.isLagging) {
                        std::cerr << "[Network Telemetry] Warning: TCP stall detected! OS buffer "
                                     "backed up with "
...
void BufferedMjpegServer::start() {
    ...
    networkThread_ = std::thread([this, &loopPromise]() {
        ...
        app.listen(port_, [this](us_listen_socket_t* socket) {
            if (socket) {
                ...
                us_timer_t* timer = us_create_timer(loop, WebServerConfig::TIMER_FALLTHROUGH,
                                                    sizeof(BufferedMjpegServer*));

                *static_cast<BufferedMjpegServer**>(us_timer_ext(timer)) = this;
                us_timer_set(timer, BufferedMjpegServer::onTimer,
