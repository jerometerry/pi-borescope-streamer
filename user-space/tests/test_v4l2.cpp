...
class V42LTest : public ::testing::Test {
   ...
   protected:
    ...
    Arguments::ParseResult parseArguments() {
        ...
        std::transform(args_.begin(), args_.end(), std::back_inserter(ptr_vector),
...
TEST_F(V42LTest, ParseDevicePath) {
    std::vector<std::string> args = {"./v4l2-borescope-daemon",
                                     "--dev",
                                     "/dev/video3",
                                     "--bus",
                                     "1",
...
TEST_F(V42LTest, ParseBusLargerThan255) {
    ...
    EXPECT_THAT([this]() { parseArguments(); },
...
TEST_F(V42LTest, ParseHelp) {
    std::vector<std::string> args = {"./v4l2-borescope-daemon", "--help"};
    addArguments(args);
    auto result = parseArguments();
...
TEST_F(V42LTest, ParseUnknownArgument) {
    std::vector<std::string> args = {"./v4l2-borescope-daemon", "--unknown", "N/A"};
    addArguments(args);
