...
BufferedMjpegServer::BufferedMjpegServer(const int port, const std::atomic<bool>& running, FrameSource frameSource)
        ...
        for (size_t i = 0; i < server->activeViewers_.size(); ) {
            ...
            if (viewer.lastSentFrameId < currentFrameId) {
                ...
                if (backpressure == 0) {
                    ...
                    CorkState state {
                        res,
                        BufferedZeroAllocationResponseBuilder::build(currentFrame.get()),
                        false
                    };
