...
class CameraHardwareEmulator {
	...
	void emit_video_frame(uint8_t			  frame_id,
			      ...
			      bool inject_gravity = false)
	{
		...
		while (offset < jpeg_data.size()) {
			size_t chunk_size = std::min<size_t>(
				932, jpeg_data.size() - offset);
			std::vector<uint8_t> chunk(jpeg_data.begin() + offset,
						   jpeg_data.begin() + offset +
							   chunk_size);
			...
			if (inject_gravity && (packet_counter % 3 == 0)) {
				...
				emit_packet(GRAVITY_SENSOR_ID, frame_id, 0x01,
					    gs_data);
			}
		}
	}
...
class UseeplusDecoderTest : public ::testing::Test {
	...
	void SetUp() override
	{
		...
		decoder.cb.on_video_frame_start = [](void *ctx, u8 fid,
						     u8 dnum) {
			static_cast<UseeplusDecoderTest *>(ctx)
				->events.push_back(
					{ DecoderEvent::START, fid, 0 });
		};
		decoder.cb.on_video_frame_fragment = [](void *ctx, u8 *data,
							size_t len) {
			static_cast<UseeplusDecoderTest *>(ctx)
				->events.push_back(
...
TEST_F(UseeplusDecoderTest, DecodesClean10KbVideoFrame)
{
	...
	size_t consumed =
		up_decode_bulk(&decoder, stream.data(), stream.size());
...
TEST_F(UseeplusDecoderTest, RecoversFromCorruptedPacketLength)
{
	...
	for (const auto &ev : events) {
		if (ev.type == DecoderEvent::COMPLETE)
			found_complete = true;
	}
